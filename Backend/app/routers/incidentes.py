...
async def consultar_incidentes(db: DBDep, current_user: CurrentUser):
    es_admin = any(rol.nombre == "ADMINISTRADOR" for rol in current_user.roles)
    return await incidente_service.consultar_historial_incidentes(current_user.id_usuario, es_admin, db)
...
@router.get(
    ...
    description="Muestra el detalle asocido a un incidente como el estado y el taller asignado"
)
async def obtener_incidente(id_incidente: int, db: DBDep, current_user: CurrentUser):
    es_admin = any(rol.nombre == "ADMINISTRADOR" for rol in current_user.roles)
