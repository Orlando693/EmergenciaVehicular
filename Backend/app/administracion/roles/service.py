...
from app.administracion.usuarios.model import Rol, Permiso, RolPermiso, Usuario, UsuarioRol
from app.administracion.roles.schemas import RolCreate, RolUpdate, RolOut, PermisoOut, PermisoCreate


def _es_rol_sistema(rol: Rol) -> bool:
    """Roles de sistema (id_tenant NULL) no pueden ser modificados por un ADMIN normal."""
    return rol.id_tenant is None


async def listar_roles(db: AsyncSession, id_tenant: int) -> list[RolOut]:
    """Devuelve roles globales (sistema) + roles propios del tenant."""
    result = await db.execute(
        select(Rol)
        .where(or_(Rol.id_tenant.is_(None), Rol.id_tenant == id_tenant))
        .options(selectinload(Rol.permisos))
    )
    return result.scalars().all()


async def crear_rol(data: RolCreate, db: AsyncSession, id_tenant: int) -> RolOut:
    existe = await db.execute(
        select(Rol).where(Rol.nombre == data.nombre.upper(), Rol.id_tenant == id_tenant)
    )
    if existe.scalar_one_or_none():
        raise HTTPException(status_code=409, detail="El rol ya existe en este tenant")
    rol = Rol(id_tenant=id_tenant, nombre=data.nombre.upper(), descripcion=data.descripcion)
    db.add(rol)
    await db.commit()
...
async def actualizar_rol(id_rol: int, data: RolUpdate, db: AsyncSession, id_tenant: int) -> RolOut:
    result = await db.execute(
    ...
    if _es_rol_sistema(rol):
        raise HTTPException(status_code=403, detail="No se puede modificar un rol del sistema")
    if rol.id_tenant != id_tenant:
        raise HTTPException(status_code=403, detail="No tienes permiso sobre este rol")
...
async def eliminar_rol(id_rol: int, db: AsyncSession, id_tenant: int) -> dict:
