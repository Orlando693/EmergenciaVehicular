...
from app.core.enums import EstadoUsuarioEnum
...
class Rol(Base):
    ...
    # NULL = rol de sistema global; NOT NULL = rol propio del tenant
    id_tenant = Column(BigInteger, ForeignKey("tenants.id_tenant", ondelete="CASCADE"), nullable=True, index=True)
...
class Usuario(Base):
    ...
    __table_args__ = (
        UniqueConstraint("id_tenant", "email", name="uq_usuarios_tenant_email"),
    )
