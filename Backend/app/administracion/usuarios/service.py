...
from app.administracion.usuarios.model import Cliente, Rol, Usuario, UsuarioRol
from app.administracion.usuarios.schemas import CambiarPasswordRequest, UsuarioCreate, UsuarioOut, UsuarioUpdate
from app.administracion.tenants.model import Tenant
from app.administracion.tenants.service import obtener_tenant_demo
...
async def registrar_usuario(data: UsuarioCreate, db: AsyncSession, id_tenant: int | None = None) -> UsuarioOut:
    # Resolver tenant: prioridad → id_tenant (admin), tenant_slug (registro público), tenant-demo (fallback)
    if id_tenant is None:
        if data.tenant_slug:
            res = await db.execute(
                select(Tenant).where(func.lower(Tenant.slug) == data.tenant_slug.strip().lower())
            )
            tenant = res.scalar_one_or_none()
            if not tenant:
                raise HTTPException(status_code=404, detail=f"Tenant '{data.tenant_slug}' no encontrado")
            if tenant.estado != "ACTIVO":
                raise HTTPException(status_code=403, detail="El tenant indicado no está activo")
    ...
    existe = await db.execute(
        select(Usuario).where(
            func.lower(Usuario.email) == data.email.lower(),
