...
from app.operaciones.talleres.schemas import TallerCreate, TallerEstadoUpdate, TallerOut, TallerUpdate
...
async def listar_talleres(
    ...
    query = select(Taller).where(Taller.id_tenant == id_tenant)
    if estado:
...
async def obtener_taller(id_taller: int, db: AsyncSession, id_tenant: int) -> TallerOut:
...
async def obtener_taller_por_usuario(id_usuario: int, db: AsyncSession, id_tenant: int) -> TallerOut:
    result = await db.execute(select(Taller).where(Taller.id_usuario == id_usuario, Taller.id_tenant == id_tenant))
    taller = result.scalar_one_or_none()
    if not taller:
...
async def actualizar_taller(id_taller: int, data: TallerUpdate, db: AsyncSession, id_tenant: int) -> TallerOut:
    result = await db.execute(select(Taller).where(Taller.id_taller == id_taller, Taller.id_tenant == id_tenant))
    taller = result.scalar_one_or_none()
    if not taller:
        raise HTTPException(status_code=404, detail="Taller no encontrado")
...
async def obtener_metricas_taller(id_usuario: int, id_tenant: int, db: AsyncSession) -> dict:
    t_res = await db.execute(select(Taller).where(Taller.id_usuario == id_usuario, Taller.id_tenant == id_tenant))
    ...
    sub_inc = select(Incidente.id_incidente).where(
        Incidente.id_tenant == id_tenant,
        Incidente.id_taller == taller.id_taller,
    )
    r_ing = await db.execute(
        select(func.sum(Pago.monto_taller), func.sum(Pago.comision_plataforma), func.count())
        .where(Pago.id_tenant == id_tenant, Pago.id_incidente.in_(sub_inc), Pago.estado == "COMPLETADO")
    )
    row = r_ing.one()
...
async def obtener_pagos_taller(id_usuario: int, id_tenant: int, db: AsyncSession) -> list[dict]:
    t_res = await db.execute(select(Taller).where(Taller.id_usuario == id_usuario, Taller.id_tenant == id_tenant))
    taller = t_res.scalar_one_or_none()
    if not taller:
        raise HTTPException(status_code=404, detail="Taller no encontrado para este usuario")
    ...
    r_pagos = await db.execute(
        ...
        .where(Pago.id_tenant == id_tenant, Pago.id_incidente.in_(sub_inc))
