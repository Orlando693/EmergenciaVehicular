...
from app.gestion_incidentes.incidentes.schemas import IncidenteOut, IncidenteCreate, IncidenteHistorialOut, IncidenteEstadoUpdate
from app.gestion_incidentes.incidentes import service as incidente_service
from app.asignacion_atencion.solicitudes.service import asignar_taller_optimo
...
async def upload_file(
    ...
    """
    tenant_dir = os.path.join(settings.UPLOAD_DIR, str(current_user.id_tenant))
    ...
    return {"url": f"{settings.UPLOAD_URL_PREFIX}/{current_user.id_tenant}/{filename}"}
