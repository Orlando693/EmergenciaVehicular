...
export class HomeComponent implements OnInit {
  ...
  private cargarStatsAdmin() {
    this.reportesSrv.resumen().subscribe({
      next: (r) => {
        ...
        this.chartItems.set([
          { label: 'Reportados', value: r.incidentes_reportados ?? 0, color: 'blue' },
          { label: 'En proceso', value: r.incidentes_en_proceso ?? 0, color: 'yellow' },
          { label: 'Pagos', value: r.total_pagos_completados ?? 0, color: 'green' },
          { label: 'Clientes', value: r.total_clientes ?? 0, color: 'teal' },
        ]);
        this.loading.set(false);
      },
  ...
  private cargarStatsTaller() {
    this.tallerSrv.misMetricas().subscribe({
      next: (m) => {
        ...
        this.chartItems.set([
          { label: 'Recibidos', value: m.solicitudes_recibidas ?? 0, color: 'blue' },
          { label: 'En proceso', value: m.en_proceso ?? 0, color: 'yellow' },
          { label: 'Completados', value: m.completados ?? 0, color: 'green' },
          { label: 'Cancelados', value: m.cancelados ?? 0, color: 'teal' },
        ]);
        this.loading.set(false);
      },
      error: () => this.loading.set(false),
  ...
  private cargarStatsCliente() {
    this.incidenteSrv.misMetricasCliente().subscribe({
      next: (m) => {
        ...
        this.chartItems.set([
          { label: 'Reportados', value: m.reportados ?? 0, color: 'blue' },
          { label: 'En atencion', value: m.en_proceso ?? 0, color: 'yellow' },
          { label: 'Pagados', value: m.pagados ?? 0, color: 'green' },
          { label: 'Cancelados', value: m.cancelados ?? 0, color: 'teal' },
        ]);
        this.loading.set(false);
      },
  ...
  chartWidth(value: number): number {
    return Math.max(6, Math.round((value / this.chartMax()) * 100));
  }

  actionTitle(): string {
    if (this.auth.rol === 'CLIENTE') return 'Generar diagnostico IA';
    if (this.auth.rol === 'TALLER') return 'Revisar solicitudes';
    return 'Supervisar operaciones';
  }

  actionRoute(): string {
    if (this.auth.rol === 'CLIENTE') return '/dashboard/incidentes/nuevo';
    if (this.auth.rol === 'TALLER') return '/dashboard/solicitudes-disponibles';
    return '/dashboard/incidentes';
  }

  actionDescription(): string {
    if (this.auth.rol === 'CLIENTE') return 'Reporta una emergencia, adjunta evidencia y recibe una orientacion inmediata.';
    if (this.auth.rol === 'TALLER') return 'Atiende solicitudes disponibles y mantén tus servicios al dia.';
    return 'Revisa incidentes, usuarios y talleres desde un solo punto.';
  }
}
