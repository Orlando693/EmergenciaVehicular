  ...
  template: `
    <div class="bg-white shadow rounded-lg p-6">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Historial de Incidentes</h2>
      </div>
      ...
      <div *ngIf="!cargando() && incidentes().length === 0" class="text-center p-8 border-2 border-dashed border-gray-300 rounded-lg bg-gray-50">
        ...
        <h3 class="text-lg font-medium text-gray-900">No hay incidentes registrados</h3>
        <p class="mt-1 text-gray-500">Aún no existe historial de incidentes en el sistema.</p>
      </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr *ngFor="let inc of incidentes()" class="hover:bg-gray-50 transition">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <button (click)="verDetalle(inc.id_incidente)" class="text-blue-600 hover:text-blue-900 bg-blue-50 hover:bg-blue-100 px-3 py-1 rounded transition">
                  Ver detalle
                </button>
              </td>
            </tr>
...
export class IncidentesListComponent implements OnInit {
  ...
  cargarHistorial() {
    ...
    this.incidenteService.consultarHistorial().subscribe({
      ...
      error: () => {
        this.error.set('No se pudo cargar el historial de incidentes.');
        this.cargando.set(false);
      }
    });
  }
  ...
  verDetalle(id: number) {
    this.router.navigate(['/dashboard/incidentes/detalle', id]);
  }
}
