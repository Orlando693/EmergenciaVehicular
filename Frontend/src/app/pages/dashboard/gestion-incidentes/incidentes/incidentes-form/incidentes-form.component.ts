...
export class IncidentesFormComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private scheduleMapInit(delay = 150) {
  ...
  private initMap() {
    ...
    if (!container) {
      if (this.mapInitAttempts < 20) {
        ...
        this.scheduleMapInit(250);
        return;
      }
      this.mapMessage.set('El mapa no esta disponible. Puedes generar el diagnostico sin ubicacion.');
      return;
    }
    this.mapInitAttempts = 0;
    this.mapMessage.set('Cargando mapa...');
    try {
    ...
    tiles.on('load', () => {
      if (this.mapMessage() === 'Cargando mapa...') {
        this.mapMessage.set(null);
      }
    });
  ...
  cargarVehiculos() {
    this.vehiculoService.consultarVehiculos().subscribe({
      next: (data) => {
        ...
        if (data.some(v => v.activo)) {
          this.scheduleMapInit(100);
        }
      },
      error: () => {
